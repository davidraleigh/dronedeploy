...
def pitch_roll_pts(height, width, rot_y, rot_x):
    ...
    center_offset = np.matrix(
        [[center_x, center_x, center_x, center_x], [center_y, center_y, center_y, center_y], [0, 0, 0, 0]])
...
def coords_from_azi_distance(lat1_deg, lon1_deg, azimuth_deg, distance_cm):
    ...
    lat2_rad = math.asin(math.sin(lat1_rad) * math.cos(distance_km / radius_km) + math.cos(lat1_rad) * math.sin(
        distance_km / radius_km) * math.cos(azimuth_rad))
    lon2_rad = lon1_rad + math.atan2(math.sin(azimuth_rad) * math.sin(distance_km / radius_km) * math.cos(lat1_rad),
...
def envelope_from_image(img, lon1_deg, lat1_deg, x_pixel, y_pixel, GSD):
    ...
    lon_null, lat_up = coords_from_azi_distance(lat1_deg, lon1_deg, azimuths_deg[0], distances_cm[0])
    lon_right, lat_null = coords_from_azi_distance(lat1_deg, lon1_deg, azimuths_deg[1], distances_cm[1])
    lon_null, lat_down = coords_from_azi_distance(lat1_deg, lon1_deg, azimuths_deg[2], distances_cm[2])
    lon_left, lat_null = coords_from_azi_distance(lat1_deg, lon1_deg, azimuths_deg[3], distances_cm[3])
...
def roll_pitch_size(item):
    return math.fabs(float(item['Yaw est']))  #
    # return math.fabs(float(item['Roll est'])) + math.fabs(float(item['Pitch est']))


def compare(item1, item2):
    if roll_pitch_size(item1) < roll_pitch_size(item2):
        return -1
    elif roll_pitch_size(item1) > roll_pitch_size(item2):
        return 1
    else:
        return 0
...
if __name__ == "__main__":
    ...
    with open(IMAGE_DETAILS_FILE, 'r') as image_details_csv:
        ...
        for row in details_reader:
            ...
            drone_pixel_x, drone_pixel_y = calculate_drone_position_pixel(warped_image, pitch_rad, roll_rad)
            print(drone_pixel_x)
            print(drone_pixel_y)

            rotated_image = rotate_image(warped_image, -yaw_deg, (drone_pixel_x, drone_pixel_y))
            ...
            print('GSD')
            GSD = cm_per_pixel(warped_image, pitch_rad, roll_rad, elevation_meters)
            print(GSD)
            print('')
            ...
            ulx, uly, lrx, lry = envelope_from_image(rotated_image, lon_deg, lat_deg, width / 2, height / 2, GSD)

            env = "-a_ullr " + str(ulx) + " " + str(uly) + " " + str(lrx) + " " + str(lry) + " "
            gdal_tif_filename = jpg_filename + 'gdal.tif'
            os.system(
                "gdal_translate -co compress=LZW -of GTiff -a_srs EPSG:4326 -a_nodata 0 " + env + temp_filename + " " + gdal_tif_filename)
